  ...
  class AbfsListIterator implements RemoteIterator<LocatedFileStatus> {
    ...
    public LocatedFileStatus next() throws IOException {
      ...
      if (currBatchResponse.shouldLoadNextBatch(currIndex)) {
        currBatchResponse = getAbfsStore().batchListStatus(currBatchResponse.getPath(), recursive, currBatchResponse.getContinuation(), tracingContext);
        currIndex = 0;
      }
  ...
  public RemoteIterator<LocatedFileStatus> listFiles(Path f, boolean recursive)
    ...
    try {
      TracingContext tracingContext = new TracingContext(clientCorrelationId,
          ...
          listener);
      return new AbfsListIterator(getAbfsStore().batchListStatus(qualifiedPath, recursive, null, tracingContext), recursive, tracingContext);
    } catch (AzureBlobFileSystemException ex) {
      checkException(f, ex);
